...
K8S_MODULE_LOCATION=$5"/Docker-Puppet/kubernetes-artifacts"
PRIVATE_DOCKERHUB=$1
...
dockerRegistryLogin(){
  docker login --username=xxxxxxx --email=xxxxxxxx@wso2.com --password=$password_decoded dockerhub.private.wso2.com
}
...
then echo "Executing default"
     ...
     cd $K8S_MODULE_LOCATION"/common/wso2-shared-dbs"
     sh deploy.sh
     cd -
     kubectl create -f $K8S_MODULE_LOCATION"/wso2am/mysql-apimdb-controller.yaml"
     kubectl create -f $K8S_MODULE_LOCATION"/wso2am/mysql-apimdb-service.yaml"
     fi
     ...
     #docker push  http://$PRIVATE_DOCKERHUB/dimuthud-wso2-am-api-publisher:1.10.0
     #docker push  http://$PRIVATE_DOCKERHUB/dimuthud-wso2-am-api-store:1.10.0
     #docker push  http://$PRIVATE_DOCKERHUB/dimuthud-wso2-am-api-gateway:1.10.0
     #docker push  http://$PRIVATE_DOCKERHUB/dimuthud-wso2-am-api-keymanager:1.10.0

     echo writing to text file docker image details


     #mkdir $TEST_RESOURCE_LOCATION"/artifacts/AM/scripts/bashscripts"
     touch FILE
     /bin/cat <<EOM >$FILE
     wso2am-api-publisher: image: $PRIVATE_DOCKERHUB/dimuthud-wso2-am-api-publisher tag:1.10.0
     wso2am-api-store: image:$PRIVATE_DOCKERHUB/dimuthud-wso2am-api-store tag:latest
