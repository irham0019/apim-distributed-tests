...
public class APIScopeTestCase extends APIMIntegrationBaseTest {
    ...
    public void testSetScopeToResourceTestCase() throws Exception {
        ...
        if (keyManagerContext.getContextTenant().getDomain().equals("carbon.super")) {
            gatewayUrl = gatewayUrlsMgt.getWebAppURLNhttp();
            userJohn = USER_JOHN;
        } else {
            gatewayUrl = gatewayUrlsMgt.getWebAppURLNhttp() + "t/" + keyManagerContext.getContextTenant().getDomain() + "/";
        ...
        URL tokenEndpointURL = new URL(gatewayUrlsMgt.getWebAppURLNhttp() + "token");
        String accessToken;
